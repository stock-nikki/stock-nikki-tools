...
| `stock_karte_template_v1.md` | 銘柄カルテのテンプレと運用ルール | カルテ新規作成時 ※実体は **stocks-karte** リポジトリ（プライベート）のREADMEに配置 |
| `screening_workflow_v1.md` | 2段階スクリーニング（チャッピー→クロ）の方針とプロンプト | 月次/四半期のスクリーニング実行時 |
...
|---|---|---|
| A | 週次ポートフォリオ更新 | vol1 |
| B | S株入口記事（初心者向け解説） | vol1 |
| C | 保有銘柄振り返り | vol1 |
| D | 日次短報 | vol1 |
| E | 個別銘柄分析（非保有） | vol2 |
| F | 決算読み解き | vol2 |
| G | 配当・優待記事 | vol2 |
| H | クロvsチャッピー銘柄バトル | vol3 |
| I | AI活用記事 | vol3 |
| J | 投資イベントカレンダー | vol4 |
| K | 自己破産からの再起ログ | vol4 |
| L | 体験レビュー | vol4 |

## 🔧 自作ツール対応表（名前を混同しないためｗ）

| ツール | 用途 | 使う場面 |
|---|---|---|
| `dividend_fetcher.py` | 配当データ取得（--detailで配当継続性など詳細指標） | 配当記事（パックG）の素材 |
| `japan-stock-radar.py` | 全銘柄（約3,700）ファンダメンタルTSV生成 | 2段階スクリーニングの起点 |
| `stock-chart-all.html` | ポートフォリオ可視化チャート | 週次更新（パックA） |
...
### 【カルテ】銘柄カルテ作成（買う前・義務）※パック外・stocks-karte運用
...
### 【スクリーニング】2段階スクリーニング（月次/四半期）※パック外・screening_workflow運用

**Stage 1はチャッピーに**：`screening_workflow_v1.md` 内のStage 1プロンプトをコピペ＋全銘柄TSV添付
（プロンプト内に**保有銘柄コードの除外リスト**を貼る。クロちゃんに「保有コード一覧ちょうだい」でOK。候補100枠は新規専用）
...
→ 新規30銘柄のTier分け＋保有銘柄の買い増し推奨（同一スコア基準で比較）＋「今月は幅か厚みか」の所見が返ってくる
