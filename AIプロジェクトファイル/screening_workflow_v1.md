# 2段階スクリーニング・ワークフロー v1.3

> 2026-07-05 fable5策定（v1.1：候補100枠を新規専用化／v1.2：52週位置の参考列を追加／v1.3：判定ログのコードブロック出力を義務化）。
...
【出力の重要ルール】上記1と3（判定ログ・通過数・業種分布）は、**必ず1つのMarkdownコードブロック（```で囲む）にまとめてプレーンテキストで出力**すること。
...
3. **定性評価**：業種ノーム照合（商社の低PERは正常等）・ビジネスモデル・競争優位・罠フラグの個別検証。
   52週位置(%)は「安値圏＝放置か悪材料かの見極め」「高値圏＝焦って買う必要があるかの確認」に使う（位置そのものは合否にしない）
